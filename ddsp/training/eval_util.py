        ...
        for i in range(batch_size):
            ...
            if is_outlier(feats['f0_confidence']):
                ...
                if f0_encoder.shape[0] != f0_original.shape[0]:
                    f0_encoder = ddsp.core.resample(
                        f0_encoder, f0_original.shape[0])
                ...
                if f0_crepe_dist is None or f0_crepe_dist > OUTLIER_MIDI_THRESH:
                    ...
                    self.metrics['f0_crepe_outlier_ratio'].update_state(
                        True, True)
...
def f0_summary(f0_hz, f0_hz_predict, step, name=''):
    ...
    for i in range(batch_size):
        ...
        if f0_midi_predict.shape[0] != f0_midi.shape[0]:
            f0_midi_predict = ddsp.core.resample(
                f0_midi_predict, f0_midi.shape[0])
        ...
        for checkpoint_path in checkpoints_iterator:
            ...
            for batch_idx in range(1, num_batches + 1):
                try:
                    ...
                    logging.info('Predicting batch %d of size %d',
                    ...
                    if mode == 'sample':
                        start_time = time.time()
                        logging.info(
                            'Writing summmaries for batch %d', batch_idx)
                        ...
                        start_time = time.time()
                        logging.info(
                            'Calculating metrics for batch %d', batch_idx)
