...
def hz_to_midi(frequencies: Number) -> Number:
    ...
    def log2(x): return tf.math.log(x) / tf.math.log(2.0)
...
def upsample_with_windows(inputs: tf.Tensor,
    ...
    # [batch_size, n_channels, n_frames]
    ...
    # Transpose back.
    # [batch_size, n_timesteps, n_channels]
...
def wavetable_synthesis(frequencies: tf.Tensor,
    ...
    # Create sample-wise envelopes.
    amplitude_envelope = resample(
        amplitudes, n_samples, method='window')[:, :, 0]
